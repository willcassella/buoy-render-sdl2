...
impl Element for BlueBox {
    fn run<'ctx, 'frm>(self, mut ctx: Context<'ctx, 'frm>, id: Id) -> LayoutNode<'frm> {
        ...
        {
            Overlap::build(id.append_str("overlap")).begin(&mut sub);
            {
                Border::build(id.append_str("border"))
                    ...
                    .begin(&mut sub);
                {
                    Fill::build(fill_id).color(inner_color).begin(&mut sub);
                    sub.end(); // fill
                }
                sub.end(); // border

                // Create a hover state
                Hover::build(id.append_str("hover"), hover_out).begin(&mut sub);
                sub.end(); // hover
            }
            sub.end(); // overlap
        }
...
impl Element for Repeating {
    fn run<'ctx, 'frm>(self, mut ctx: Context<'ctx, 'frm>, id: Id) -> LayoutNode<'frm> {
        let mut sub = ctx.open_sub(
            ctx.max_area(),
            (id.append_str("list"), List::left_to_right()),
        );
        {
            for i in 0..10 {
                    ...
                    .begin(&mut sub);
                {
                    sub.begin(
                        SocketName::default(),
                        (id.append_str("BlueBox").append_num(i), BlueBox),
                    )
                    .end();
                }
                sub.end(); // padding
    ...
    fn run<'ctx, 'frm>(self, mut ctx: Context<'ctx, 'frm>, id: Id) -> LayoutNode<'frm> {
        let grid = Grid {
            ...
            regions: vec![
                GridRegion::col_span(
                    SocketName::from("top"),
                    ColIndex::from_left(0),
                    ColIndex::from_right(0),
                    RowIndex::from_top(0),
                ),
                GridRegion::cell(
                    SocketName::from("one"),
        ...
        let mut sub = ctx.open_sub(ctx.max_area(), (id.append_str("grid"), grid));
        {
            ...
            sub.begin(
                SocketName::from("bot"),
                (id.append_str("bot"), Fill::new(color::constants::BLUE)),
            )
            .end();
        }
        sub.close()
